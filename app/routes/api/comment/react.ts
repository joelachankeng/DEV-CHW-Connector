...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const result =
    action === "REMOVE"
      ? await Feed.API.Comment.unReactComment(userId, postId, emojiId)
      : await Feed.API.Comment.reactComment(userId, postId, emojiId, emojiIcon);
